...
const experiences = [
  {
    ...
    current: true,
    color: '#c9985a',
    description: 'Leading IT operations and network infrastructure management across a 101-location national salon franchise, ensuring reliable technology for day-to-day business operations.',
    bullets: [
  ...
  },
  {
    ...
    current: false,
    color: '#a07848',
    description: 'Contributed to a cross-functional development team building an Android application that converts unstructured voice and text input into structured, actionable data.',
    bullets: [
  ...
  },
  {
    ...
    current: false,
    color: '#8b6433',
    description: 'Provided professional administrative and communication support in a fast-paced virtual environment, maintaining organized workflows and responsive service.',
    bullets: [
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 reveal">
          ...
          <p className="section-subheading mx-auto">
            Real-world roles where I've applied software development and IT expertise to keep technology reliable and business operations running.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(to bottom, rgba(201,152,90,0.35), rgba(139,100,51,0.35), transparent)' }}
          />

          <div className="space-y-12">
            {experiences.map((exp, i) => (
              <div key={exp.company} className={`reveal reveal-delay-${i + 1} relative grid md:grid-cols-2 gap-6 md:gap-12`}>
                {/* Timeline dot */}
                <div
                  className="absolute left-0 md:left-1/2 top-6 w-4 h-4 rounded-full border-2 -translate-x-1/2 hidden md:block z-10"
                  style={{ borderColor: exp.color, background: '#0c0906', boxShadow: `0 0 12px ${exp.color}66` }}
                />
                ...
                <div className={`${i % 2 === 0 ? 'md:col-start-1' : 'md:col-start-2'} col-span-1`}>
                  <div
                    ...
                    onMouseEnter={(e) => (e.currentTarget.style.boxShadow = `0 0 40px ${exp.color}18`)}
                    onMouseLeave={(e) => (e.currentTarget.style.boxShadow = '')}
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-bold text-lg" style={{ color: '#f0ebe2' }}>{exp.title}</h3>
                          {exp.current && (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: exp.color, color: '#0c0906' }}>
                              Current
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex flex-wrap gap-3 mb-4 text-xs" style={{ color: '#6b5e52' }}>
                      <span className="flex items-center gap-1"><Calendar size={12} /> {exp.period}</span>
                      <span className="flex items-center gap-1"><MapPin size={12} /> {exp.location}</span>
                    </div>

                    <p className="text-sm leading-relaxed mb-4" style={{ color: '#9e8e7e' }}>{exp.description}</p>

                    <ul className="space-y-2 mb-5">
                      {exp.bullets.map((b, bi) => (
                        <li key={bi} className="flex items-start gap-2 text-sm" style={{ color: '#c8bfb2' }}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.tags.map((tag) => (
                        <span
                          ...
                          style={{ background: `${exp.color}10`, border: `1px solid ${exp.color}22`, color: exp.color }}
                        >
                          {tag}
                        </span>
                      ))}
