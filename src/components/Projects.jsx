...
const projects = [
  {
    ...
    description: 'An Android application that transforms unstructured voice and text inputs into structured, actionable military reports — enhancing situational awareness and operational efficiency in real-world defense scenarios.',
    features: [
  ...
  },
  {
    ...
    description: 'This portfolio website — built with React, Vite, and Tailwind CSS — showcasing a clean, responsive, and professional personal brand with smooth animations and a polished warm dark UI.',
    features: [
...
function ProjectCard({ project }) {
  return (
    <div
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.borderColor = `${project.color}38`
        e.currentTarget.style.boxShadow = `0 0 50px ${project.color}14`
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              {project.featured && (
                <span className="px-2 py-0.5 rounded text-xs font-bold" style={{ background: project.color, color: '#0c0906' }}>
                  Featured
                </span>
              )}
            </div>
            <h3 className="font-bold text-xl mb-1 transition-colors" style={{ color: '#f0ebe2' }}>{project.title}</h3>
          ...
          <div className="flex items-center gap-2 ml-4 flex-shrink-0">
            {project.github && project.github !== '#' && (
              <a href={project.github} target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: 'rgba(255,255,255,0.04)', color: '#9e8e7e' }}
        ...
        <p className="text-sm leading-relaxed mb-5" style={{ color: '#9e8e7e' }}>{project.description}</p>

        <ul className="space-y-2 mb-6 flex-1">
          {project.features.map((f, i) => (
            <li key={i} className="flex items-start gap-2 text-sm" style={{ color: '#c8bfb2' }}>
...
export default function Projects() {
  return (
    <section id="projects" className="py-24 relative overflow-hidden">
      <div className="absolute inset-0" style={{ background: 'rgba(14,9,4,0.40)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 reveal">
          ...
          <p className="section-subheading mx-auto">
            A selection of projects that demonstrate my ability to design, develop, and ship real software solutions.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-6">
          {projects.map((project, i) => (
            <div key={project.title} className={`reveal reveal-delay-${i + 1}`}>
        ...
        <div className="text-center mt-12 reveal">
          <p className="mb-4" style={{ color: '#9e8e7e' }}>More projects are in progress. Check back soon or view my GitHub.</p>
          <a href="https://github.com/AmerKadric" target="_blank" rel="noopener noreferrer" className="btn-outline inline-flex">
            <Github size={16} />
            View GitHub Profile
          </a>
        </div>
